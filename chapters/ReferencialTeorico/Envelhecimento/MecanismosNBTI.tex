...
Cada um desses fenômenos contribui para a variação da tensão de threshold resultando na Equação \ref{eq:SomaVth}. Onde $\Delta V\scriptstyle{IT}$ é a contribuição das armadilhas na interface (primeiro mecanismo), $V\Delta \scriptstyle{HT}$ é a contribuição do aprisionamento em defeitos pré existentes e $V\Delta \scriptstyle{OT}$ é a contribuição do aprisionamento em defeitos gerados eletricamente (segundo mecanismo).
